...
This report is particularly geared toward the state and local administrative datasets and includes guidance for the issues common to these datasets. The design of this report was developed based on testing on state data for the Temporary Assistance for Needy Families (TANF) program. However, other data files with similar formats may benefit from the use of the toolkit.  

When a strange result or pattern is detected in these analyses, it is often difficult to know whether the pattern reflects an issue with the data or just a notable pattern, for example changes in benefit amounts due to legal changes. While the toolkit will often not be able to distinguish between data issues and other notable patterns, observing these patterns can be greatly informative for understanding your data and guiding interpretations of research findings. Further, understanding the program details affecting the dataset analyzed can great assist with interpretation of findings in this report.  
...
This report is generated using R Markdown, available with R and RStudio. The sections of this report include R code chunks that refer to different code needed to implement analyses. Convenient defaults are included for this code for its for TANF data. We encourage the user to modify the script to the specific needs of their analyses. For each section, the R script to refer to for making changes is designated by the "**Reference Script**."  

Places where we recommend the user consider modifying the code are designated in the report below and are also designated in "Guidance_for_User_Input.xlsx". We also have set up the R code to read inputs from modifying "setup.yml." Thus, a user can make modifications to this input file for key inputs for the toolkit in place of modifying the R code.  

For troubleshooting relating to output for this report, please see the instructions in the User Documentation and refer to R documentation for more specific issues.  
...
2. **Code and Output**
    i) i.	A "code chunk" is provided that demonstrates the R code used to run the analysis on the dataset. While the code includes convenient defaults for analyses of TANF data, guidance is also provided for how a user may adapt aspects of the analysis to better fit their needs.
...
Different analyses may be more or less important based on the user's research questions. For example, if the user is particular interested in cross-sectional analysis making comparisons in outcomes among groups, the comparability dimension may be particularly important.  
...
| | 1.3. Are the units included in the data set relevant to the study? | In many cases, the unit of the administrative data  is different from the relevant unit of analysis. Usually, administrative data   is at the event level such as the payment to a welfare recipient, which is   different from a database at the recipient level.
| 2. Population | 2.1. Is the population of units in the data set what   is needed for the study? | Administrative data stores detailed information of a particular group that is of interest for the administration of the activity or program. However, this group may or may not be the relevant population of   analysis.
| 3. Variables | 3.1. What variables in the data capture conceptually   what is of interest in studying? | For example, whether an income variable in the   dataset captures the right income concept of interest for research.
...
| | 4.2. Is there administrative delay in the data? | In some cases data are recorded sometime after an   event occurs. For example, the employment status of a TANF recipient could change in practice, but this might not be reflected until the next time the TANF program data are updated with data from the employment records system.
...
|  6. Policy changes | 6.1 What changes in policy and in law can you identify during the time reference?  6.2 Does any of these changes impact your analysis? | For example, when there are major changes to the tax   code, the data collected on tax forms can change and may impact time series   estimates constructed from income tax records  
...
The initially provided code is based on case-level data for a state for TANF, and variables in the corresponding member-level file are aggregated to case-level and merged into the input file. A user can input their own flat file for use for the toolkit, with requirements describe in the User Documentation. That file will need to ultimately be read to the object "merged_universe."  
...
case_level_df <- fread("insert_path_to_case_file_here")

### Read member-level file for a particular state
member_level_df <- fread("insert_path_to_member_file_here")

### Aggregates member-level data to the case level and then merges aggregates to the case-level file
#Creates indicator for adults who are TANF recipients
member_level_df$recip_adult[(member_level_df$position>=1 & member_level_df$position<=6) & member_level_df$affil == 1] <- 1
member_level_df$recip_adult[is.na(member_level_df$recip_adult)] <- 0
# Creates indicator for children who are TANF recipients
member_level_df$recip_child[(member_level_df$position>=7 & member_level_df$position<=16) & member_level_df$affil == 1] <- 1
member_level_df$recip_child[is.na(member_level_df$recip_child)] <- 0
# Aggregate to the case-level
num_recip_child <- member_level_df[, .(num_recip_child = sum(recip_child)), by = c("fips","caseid","rptmn")]
num_recip_adult <- member_level_df[, .(num_recip_adult = sum(recip_adult)), by = c("fips","caseid","rptmn")]
# Merge aggregates to the case-level file
merged_universe <- merge(case_level_df, num_recip_child, by=c("fips","caseid","rptmn"))
merged_universe <- merge(merged_universe, num_recip_adult, by=c("fips","caseid","rptmn"))

```

*Creating variables*

In this section, the user has the option to create variables that you might need for further analysis. In this section, we show examples of creating a few variables we found valuable for analyzing the TANF data:
* year which represents of the year of the case
* num_par which represents the number of adult recipients
* tot_recip which represents the total number of recipients  
...
##### EDIT THIS SECTION TO CREATE NEW VARIABLES ##### 
merged_universe$year <- floor(merged_universe[["rptmn"]]/100)
merged_universe$num_par[merged_universe$num_recip_child>0 & merged_universe$num_recip_adult==0] <- 0
merged_universe$num_par[merged_universe$num_recip_child>=0 & merged_universe$num_recip_adult==1] <- 1
merged_universe$num_par[merged_universe$num_recip_child>=0 & merged_universe$num_recip_adult>=2] <- 2
merged_universe$tot_recip <- merged_universe$num_recip_child + merged_universe$num_recip_adult

# ADDING NEW VARIABLES TO YAML CODEBOOK
new_classifications <- c("domain", "key", "time")
new_types <- c("categorical", "numeric", "numeric")
new_names <- c("num_par", "tot_recip", "year")
variables_dataframe_filled <- rbind(data.frame(classification=new_classifications,type=new_types,name=new_names, stringsAsFactors = FALSE),variables_dataframe_filled)
#####################################################
```

*Defining relevant variables*

In this section, the user should define the relevant variables for the analysis. At least one ID, key and domain variables are required, which should be indicated in the id_vars, key_vars, and domain_vars sets below. Then, the user should identify all the character variables in the charvars set below, which are all variables who are neither numeric nor dates, including IDs. It is important to make sure that only variables that were identified in the previous step are included here. Finally, the character variables are converted into factors and the user should indicate the labels of these variables where appropriate.    

```{r define_vars, results='hide'}

# Setting relevant variables as defined by yaml
...
merged_universe <- select(merged_universe,c(id_vars, time_vars, location_vars, key_vars, domain_vars))
...
merged_universe <- merged_universe[, (to_categories):=lapply(.SD, as.factor), .SDcols = to_categories]
merged_universe <- merged_universe[, (to_categories):=lapply(.SD, as.character), .SDcols = to_categories]

##### EDIT THIS SECTION TO DEFINE LABELS OF CHARACTER VARIABLES  #####
merged_universe$funding <- factor(merged_universe$funding, levels=c(1,2) , labels = c("Federal","State"))
merged_universe$rec_fs <- factor(merged_universe$rec_fs, levels=c(1,2) ,  labels = c("Yes", "No"))
merged_universe$num_par <- factor(merged_universe$num_par, levels=c(0,1,2) , labels = c("Child-only", "1-parent", "2-parent"))
...
merged_universe <- as.data.frame(merged_universe)
merged_universe[,numvars] <- sapply(merged_universe[,numvars],as.numeric)
str(merged_universe)
...
subset_param <- "rptmn>=200810 & rptmn<=201609"
########################################
...
subsetted <- try( subset(merged_universe,eval(parse(text = paste0(subset_param)))), silent=TRUE)
if (class(subsetted) == "try-error") {
  print("Subsetting parameters are not correct. Using all the data.")
  subset_text <- "All"
  subsetted <- merged_universe
}

merged_universe <- subsetted

```

# Accuracy and Completeness

```{r tech_checks, child='TechChecks/Technical_Checks.rmd', echo=TRUE}
```
