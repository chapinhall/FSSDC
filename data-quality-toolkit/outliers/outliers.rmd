...
num.file <- "dq_num.csv"
cat.file <- "dq_cat.csv"
checkDataQuality(data= merged_universe[,c(key_vars, domain_vars,id_vars,time_vars,location_vars)], out.file.num= num.file, out.file.cat= cat.file)
...
for (i in 1:length(testnums)) {
  min <- min(merged_universe[,testnums[i]],na.rm = T)
  max <- max(merged_universe[,testnums[i]],na.rm = T)
...
for (i in c(1:length(contvars))) {
  ...
  if (length(unique(merged_universe[[contvars[i]]])) < 30) {
    tablecont <-kable(table(merged_universe[[contvars[i]]],useNA="ifany",dnn=c(contvars[i])))
    print(tablecont)
  } else {
    print(histogram(merged_universe[[contvars[i]]],xlab=contvars[i]))
  }
  desc <- summary(merged_universe[[contvars[i]]])
  iqr <- desc["3rd Qu."] - desc["1st Qu."]
  
  cat("\n")
  print(kable(table(merged_universe[[contvars[i]]]> desc["3rd Qu."]+1.5*iqr,dnn="Mild High Outlier")))
  maxout<- merged_universe[which(merged_universe[[contvars[i]]]> desc["3rd Qu."]+1.5*iqr),]
  print(paste("Number of mild high outliers based on quartile method for variable",contvars[i],"is",nrow(maxout)))
  print(kable(table(merged_universe[[contvars[i]]]> desc["3rd Qu."]+3*iqr,dnn="Extreme High Outlier")))
  maxout<- merged_universe[which(merged_universe[[contvars[i]]]> desc["3rd Qu."]+3*iqr),]  
  print(paste("Number of extreme high outliers based on quartile method for variable",contvars[i],"is",nrow(maxout)))

  print(kable(table(merged_universe[[contvars[i]]]< desc["1st Qu."]-1.5*iqr,dnn="Mild Low Outlier")))
  minout <- merged_universe[which(merged_universe[[contvars[i]]]< desc["1st Qu."]-1.5*iqr),]
  print(paste("Number of mild low outliers based on quartile method for variable",contvars[i],"is",nrow(minout)))
  print(kable(table(merged_universe[[contvars[i]]]< desc["1st Qu."]-3*iqr,dnn="Extreme Low Outlier")))
  minout <- merged_universe[which(merged_universe[[contvars[i]]]< desc["1st Qu."]-3*iqr),]
  print(paste("Number of extreme low outliers based on quartile method for variable",contvars[i],"is",nrow(minout)))

  
  boxplot(merged_universe[[contvars[i]]],main=paste("Boxplot for",contvars[i],"variable"))
  
  LVboxplot(merged_universe[!is.na(merged_universe[[contvars[i]]]),contvars[i]], alpha = 0.95, k = NULL, perc = NULL,
...
lv_table <- lvtable(merged_universe[!is.na(merged_universe[[contvars[i]]]),contvars[i]],k=determineDepth(nrow(merged_universe[!is.na(merged_universe[[contvars[i]]]),]),alpha=.05),alpha=.95)
...
if (any(lv_table[['lv_thresh_h']]==1)) {
  ...
  print(kable(table(merged_universe[[contvars[i]]]>=lv_threshval_h,dnn="Letter Value High Outliers")))
  print(paste0("Number of letter value high outliers for variable ",contvars[i]," is ",nrow(subset(merged_universe,get(contvars[i])>=lv_threshval_h)),".  Values above ",lv_threshval_h," are considered outliers with this method."))
}  else {
...
if (any(lv_table[['lv_thresh_l']]==1)) {
  ...
  print(kable(table(merged_universe[[contvars[i]]]<=lv_threshval_l,dnn="Letter Value Low Outliers")))
  print(paste0("Number of letter value low outliers for variable ",contvars[i]," is ",nrow(subset(merged_universe,get(contvars[i])<=lv_threshval_l)),".  Values below ",lv_threshval_l," are considered outliers with this method."))
}  else {
