...
In this section, the user has the option to create variables that you might need for further analysis. The example provided creates a variable year based upon the month variable provided in the example data file. Note that for each new variable created, the variable type, classification, and name will be needed to add to the objects 'new_classifications', 'new_types', and 'new_names', in vector format. If no new variables are created, each of these three objects will need to be set to NULL.
...
# ADDING NEW VARIABLES ACCORDING TO SETUP IN setup.yml YAML CODEBOOK
# 'new_classifications' is a vector indicating whether each new variable is id, location, time, 
# key, # or domain
# 'new_types' is a vector indicating whether each new variable is numeric or categorical
# 'new_names' is a vector providing the new variable names
# Set 'new_classifications', 'new_types', and 'new_names' to NULL if there are 
# no newly created variables
