...
We use tableplots (Tennekes et al., 2011) as a multivariate visualization tool to help assess relationships among variables. Each variable is represented in a column, where each row is an aggregate of a given number of units. For each column, all records are sorted by values of a chosen variable in the table. The default method partitions the sort variable into percentiles and graphs its approximate distribuion by percentile. Thus the y axis presents percentiles of the sort variable.

The remaining variables have their values graphed at the corresponding values of the sort variable by row. For continuous variables, the length of the light blue bar represents the mean, and the dark blue bars represent the standard deviation (a measure of spread). For discrete variables, different colors indicate the frequency of each category.

For this analysis, we  use tableplots sorted by time. Some of the advantages of using tableplots over simple two-way graphs is that it is possible to see if the discontinuity in one variables is associated with an event in other variables, and it's also possible to analyze changes in the standard deviation of the variables.  
